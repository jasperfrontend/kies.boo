// src/hooks/useSmartCollections.tsx (Optimized with React Query)
...
const SMART_COLLECTIONS_QUERY_KEY = 'smart_collections';
...
export const useSmartCollections = (bookmarks: Bookmark[]) => {
  ...
  // Create a stable key for bookmarks to prevent unnecessary recalculations
  const bookmarksKey = useMemo(() => {
    return bookmarks.map(b => `${b.id}-${b.created_at}-${b.tags.join(',')}`).join('|');
  }, [bookmarks]);

  // Fetch database collections with React Query
  const { 
    data: dbCollections = [], 
    isLoading: loading, 
    refetch: fetchSmartCollections 
  } = useQuery({
    queryKey: [SMART_COLLECTIONS_QUERY_KEY, user?.id],
    queryFn: async () => {
      if (!user) return [];
  ...
  // Generate auto collections with caching based on bookmarks content
  const autoCollections = useQuery({
    queryKey: ['auto_collections', user?.id, bookmarksKey, deletedAutoCollectionIds.size],
    queryFn: () => {
      ...
      return generatedCollections
        ...
        .filter(collection => !deletedAutoCollectionIds.has(collection.id));
    },
    enabled: !!user && bookmarks.length >= 3,
    staleTime: 5 * 60 * 1000, // 5 minutes - longer since this is expensive
  ...
  const saveAutoGeneratedCollection = async (collection: ExtendedSmartCollection) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: [SMART_COLLECTIONS_QUERY_KEY, user.id] });
  ...
  const deleteSmartCollection = async (collectionId: string) => {
    ...
    try {
      ...
      if (isAutoGenerated) {
        // Handle dismissing auto-generated collections
        const collection = [...dbCollections, ...(autoCollections.data || [])].find(c => c.id === collectionId);
        
        if (collection && collection.bookmarks) {
          // Add "dismissed" tag to all bookmarks in this collection
          const bookmarkUpdates = collection.bookmarks.map(async (bookmark) => {
        ...
        // Invalidate auto collections cache
        queryClient.invalidateQueries({ queryKey: ['auto_collections'] });
  ...
  const allCollections = [...dbCollections, ...(autoCollections.data || [])];

  return {
    smartCollections: allCollections,
    loading: loading || autoCollections.isLoading,
    refetch: fetchSmartCollections,
