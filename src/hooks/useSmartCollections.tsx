...
export const useSmartCollections = (bookmarks: Bookmark[]) => {
  ...
  const deleteSmartCollection = async (collectionId: string) => {
    ...
    try {
      ...
      if (isAutoGenerated) {
        // For auto-generated collections, just mark them as deleted locally
        setDeletedAutoCollectionIds(prev => new Set([...prev, collectionId]));
        toast({
          title: "Success",
          description: "Smart collection removed",
        });
        return;
      }
  ...
  const autoCollections: ExtendedSmartCollection[] = useMemo(() => {
    ...
    // Filter out bookmarks that are already in saved collections
    const availableBookmarks = bookmarks.filter(bookmark => !usedBookmarkIds.has(bookmark.id));
